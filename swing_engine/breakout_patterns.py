...
def volatility_contraction(daily_df: pd.DataFrame, breakout_features: dict) -> dict:
    ...
    stage = "trigger_watch" if score >= 72 else "breakout_watch" if score >= 58 else "forming"
    return _pattern_result(
...
def flat_base(daily_df: pd.DataFrame, breakout_features: dict) -> dict:
    ...
    stage = "trigger_watch" if width <= 4.0 and score >= 68 else "breakout_watch" if score >= 54 else "forming"
    return _pattern_result(
        "flat_base",
        score,
        stage,
...
def shelf_breakout(daily_df: pd.DataFrame, breakout_features: dict) -> dict:
    ...
    stage = "trigger_watch" if width <= 5.0 and score >= 65 else "breakout_watch" if score >= 52 else "forming"
    return _pattern_result(
        "shelf_breakout",
        score,
        stage,
...
def flag_pennant(daily_df: pd.DataFrame, breakout_features: dict, continuation_pattern: dict) -> dict:
    ...
    stage = "trigger_watch" if score >= 70 else "breakout_watch" if score >= 55 else "forming"
    return _pattern_result(
        "flag_pennant",
        score,
        stage,
...
def breakout_retest(daily_df: pd.DataFrame, breakout_integrity: dict, breakout_features: dict) -> dict:
    ...
    stage = "trigger_watch" if state == "retest_holding" else "breakout_watch" if score >= 55 else "forming"
    return _pattern_result(
        "breakout_retest",
        score,
        stage,
...
def reclaim_and_go(daily_df: pd.DataFrame, daily_state: dict, breakout_features: dict) -> dict:
    ...
    stage = "trigger_watch" if abs(dist) <= 1.5 else "forming"
    return _pattern_result(
        "reclaim_and_go",
        score,
        stage,
